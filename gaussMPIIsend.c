    	...
    	/*MPI_Barrier(MPI_COMM_WORLD);*/
			...
			if (myid == 0) {
				...
				for (row = norm + 1 + myid; row < N; row += procs) {
					...
					/*Gaussian elimination*/
					/*printf("Process :- %d IN ELSE after Receive MY RANK = %d",myid,myid);
 * 					printMarix("In ELSE Initial Matrix my rank = %d:-");*/
					multiplier = A[row-1][norm] / A[norm][norm];
					for (col = norm; col < N+1; col++) {
						A[row-1][col] -= A[norm][col] * multiplier;
					}
					/*Send back the results*/
					/*printf("IN Else after Gauss : my rank %d",myid);
 * 					printMarix("IN ELSE After Matrix:-");*/
